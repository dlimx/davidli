...
const MenuFloat = styled.div<{ isDark: boolean }>`
  ...
  background: ${p => (p.isDark ? '#fafafa' : '#000000')};
  border-radius: 5px;
  font-size: 18px;
  font-weight: 600;
  ...
  &::after {
    ...
    border-top: 8px solid ${p => (p.isDark ? '#fafafa' : '#000000')};
    transition: border-color 200ms;
  }

  svg {
    path {
      fill: ${p => (p.isDark ? '#000000' : '#ffffff')};
    }
  }
`;
...
const ReferralLink: React.FC<{ disabled: boolean; share: string; children: React.ReactNode }> = ({ disabled, share, children }) => {
